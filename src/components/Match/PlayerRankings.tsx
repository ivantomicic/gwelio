...
export function PlayerRankings({ matches, currentUserId }: PlayerRankingsProps) {
  ...
  if (loading) {
    return <div className="text-gray-600 dark:text-gray-400">Loading rankings...</div>;
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Top Winners</h3>
        </div>
            ...
            topWinners.map((player, index) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-900 dark:text-white">{player.wins} wins</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{player.matches} matches</p>
                </div>
              </div>
            ))
          )}
      ...
      <div>
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Top Scorers</h3>
        </div>
        <div className="space-y-3">
          {topScorers.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400">No matches played yet</p>
            ...
            topScorers.map((player, index) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-900 dark:text-white">{player.totalPoints} points</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{player.matches} matches</p>
                </div>
              </div>
            ))
